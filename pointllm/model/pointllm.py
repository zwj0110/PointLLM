...
class PointLLMLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        logger.info(f"Point backbone RunsenXu_graspnet_enc_dec_r04 dim: {backbone_output_dim}.")
        logger.info(
        ...
        logger.info(
            f"Point projector RunsenXu_graspnet_enc_dec_r04 dim: {self.point_backbone_config['project_output_dim']}."
        )
    ...
    def initialize_tokenizer_point_backbone_config(self, tokenizer, device, fix_llm=True):
        ...
        if mm_use_point_start_end:
            ...
            if num_new_tokens > 0:
                ...
                # need to update the input embeding, but no need to update the RunsenXu_graspnet_enc_dec_r04 embedding
                ...
                if fix_llm:
                    ...
                    print(
                        f"Setting RunsenXu_graspnet_enc_dec_r04 embeddings fixed and {num_new_tokens} new tokens' input embeddings trainable."
                    )
                else:
                    ...
                    for p in self.get_output_embeddings().parameters():
                        p.requires_grad = True
                    print(
                        "Setting RunsenXu_graspnet_enc_dec_r04 embeddings and all input embeddings trainable."
                    )
