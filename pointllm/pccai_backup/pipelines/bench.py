...
def benchmark_checkpoints(opt):
    ...
    import numpy as np  # ensure available even if not imported at module level
    ...
    for filename_ckpt in opt.checkpoints:
        ...
        if opt.checkpoint_net_config is True:
        ...
        for idx, pc_file in enumerate(pc_file_list):
            ...
            # Support testing several PCs with different bit-depths
            bit_depth = opt.bit_depth[0 if len(opt.bit_depth) == 1 else idx]
            codec = get_codec_class(opt.codec_config['codec'])(opt.codec_config, pccnet, bit_depth, syntax)
            ...
            with torch.no_grad():
                # Encode
                tag = os.path.join(tmp_folder, os.path.splitext(log_dict['pc_name'])[0] + '_' + opt.exp_name)
                compressed_files, stat_dict_enc = codec.compress(pc_raw, tag=tag)

                # Decode
                if not opt.skip_decode:
                    pc_rec, stat_dict_dec = codec.decompress(compressed_files)

            # --- Metrics that do not require decode ---
            # total bits from file sizes
            log_dict['bit_total'] = np.sum([os.stat(f).st_size for f in compressed_files]) * 8
            # bpp = bits / ORIGINAL points (num_points) to reflect compression ratio against source
            # (You can switch to rec_num_points below if that's your chosen convention.)
            log_dict['bpp'] = (float(log_dict['bit_total']) / float(log_dict['num_points'])) if log_dict['num_points'] > 0 else np.nan

            # --- Metrics that require decode ---
            peak_value = opt.peak_value[0 if len(opt.peak_value) == 1 else idx]
            if opt.skip_decode:
                log_dict['d1_psnr'] = None
                log_dict['d2_psnr'] = None
                log_dict['rec_num_points'] = None
            else:
            ...
            # Optional: write reconstructed PC
            if opt.pc_write_freq > 0 and idx % opt.pc_write_freq == 0 and not opt.skip_decode:
        ...
        # ===========================
        # Averages (NaN/None-safe)
        # ===========================
        if len(log_dict_ckpt) == 0:
            logger.log.warning("No logs collected for checkpoint %s; skipping averages.", filename_ckpt)
            continue

        # Helpers to coerce to float or NaN
        ...
        bits_arr = np.array([_to_float_or_nan(ld.get('bit_total')) for ld in log_dict_ckpt], dtype=float)
        src_pts_arr = np.array([_to_float_or_nan(ld.get('num_points')) for ld in log_dict_ckpt], dtype=float)
        bpp_arr = np.array([_to_float_or_nan(ld.get('bpp')) for ld in log_dict_ckpt], dtype=float)

        # PSNR: treat missing / sentinel (<0) as NaN
        def _psnr_to_nan(x):
            v = _to_float_or_nan(x)
            return np.nan if (np.isnan(v) or v < 0) else v

        d1_arr = np.array([_psnr_to_nan(ld.get('d1_psnr')) for ld in log_dict_ckpt], dtype=float)
        if opt.compute_d2:
            d2_arr = np.array([_psnr_to_nan(ld.get('d2_psnr')) for ld in log_dict_ckpt], dtype=float)
        else:
            d2_arr = None

        # Mean of per-file bpp, ignoring NaNs
        avg_bpp = float(np.nanmean(bpp_arr)) if bpp_arr.size else np.nan

        # Global (sequence) bpp = sum(bits)/sum(points)
        total_bits = float(np.nansum(bits_arr)) if bits_arr.size else np.nan
        total_src_pts = float(np.nansum(src_pts_arr)) if src_pts_arr.size else np.nan
        seq_bpp = (total_bits / total_src_pts) if (total_src_pts and total_src_pts > 0) else np.nan

        # PSNR averages
        avg_d1_psnr = float(np.nanmean(d1_arr)) if d1_arr.size else np.nan
        avg_d2_psnr = float(np.nanmean(d2_arr)) if (opt.compute_d2 and d2_arr is not None and d2_arr.size) else np.nan

        # Build metric dict (coerce NaN -> 0.0 for logging readability)
        def _nan_to_zero(x):
            return 0.0 if (x is None or (isinstance(x, float) and np.isnan(x))) else x

        avg_metrics = {
            'bpp': _nan_to_zero(avg_bpp),
            'seq_bpp': _nan_to_zero(seq_bpp),
            'd1_psnr': _nan_to_zero(avg_d1_psnr),
        }
        if opt.compute_d2:
            avg_metrics['d2_psnr'] = _nan_to_zero(avg_d2_psnr)

        # Log current metrics for the checkpoint
