...
from ..utils.convert_octree import OctreeOrganizer
    ...
    def __init__(self, data_config, sele_config, **kwargs):
        ...
        if self.use_cache != '':
            ...
            if os.path.exists(cache_file):
                log.info("Loading pre-processed ModelNet40 cache file...")
                with open(cache_file, 'rb') as f:
                    self.cache = pickle.load(f)
            else:
                self.cache = []
                log.info("Sampling point clouds from provided dataset (PLY or OFF)...")
            ...
            log.info("ModelNet40 data loaded...\n")
