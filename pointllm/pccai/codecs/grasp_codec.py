...
class GeoResCompressionCodec(PccCodecBase):
    ...
    def __init__(self, codec_config, pccnet, bit_depth, syntax):
        ...
        if pccnet.skip_mode == False:
            pccnet.res_enc.faiss = codec_config.get('faiss',
                                                    True) and found_FAISS == True  # overwrite the option of whether to use FAISS for NN search
    ...
    def compress(self, coords, tag):
        ...
        for cnt_slice, x in enumerate(x_list):
            ...
            start = time.monotonic()
            filename_base, string_set, min_v_set, max_v_set, shape_set, scaled_num_points, base_enc_time = self.pccnet.compress(
                x, tag + '_' + str(cnt_slice))
            end = time.monotonic()
